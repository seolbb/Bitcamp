...
SELECT EMPLOYEE_ID, FIRST_NAME||' '||LAST_NAME NAME, SALARY, COMMISSION_PCT,
    ...
    SALARY * (1 + NVL(COMMISSION_PCT, 0)) * 12 SALARY12  -- NVL ==> NULL 이면 0
FROM EMPLOYEES
...
*/
SELECT DISTINCT JOB_ID FROM EMPLOYEES  -- DISTINCT 중복값 제거하고 하나만 표시
...
-- 부서별 인원을 구한 뒤 부서테이블 조인해서 부서명 확인
