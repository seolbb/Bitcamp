...
FROM CUSTOMER ORDER BY NAME;
-- 이름이 김연아면 '피겨스케이딩', 박세리면 '골프', 박지성이면 '축구'
---- 나머지는 '운동선수'
SELECT NAME,
        DECODE(NAME, '김연아', '피겨스케이팅',
                     '박세리', '골프',
                     '박지성', '축구선수', '운동선수') NAME_DECODE
FROM CUSTOMER
ORDER BY NAME;
------------------------------
...
SELECT NAME,
        DECODE(NAME, '김연아', '피겨스케이팅',
                    DECODE(NAME, '박세리', '골프',
                        DECODE(NAME, '박지성', '축구','운동선수'))) NAME_DECODE
FROM CUSTOMER
ORDER BY NAME;
...
SELECT * FROM CUSTOMER ORDER BY NAME;
-- 이름이 박지성이면 '축구선수' 아니면 '운동선수'
SELECT NAME, CASE NAME WHEN '박지성' THEN '축구선수' ELSE '운동선수' END
FROM CUSTOMER ORDER BY NAME;
------------------------------
-- 이름 김연아 -> 피겨스케이팅, 박세리 -> 골프, 박지성 -> 축구선수, 나머지 -> 운동선수
SELECT NAME, CASE NAME WHEN '김연아' THEN '피겨스케이팅'
                        WHEN '박세리' THEN '골프'
                        WHEN '박지성' THEN '축구선수'
                        ELSE '운동선수' END NAME_CASE
FROM CUSTOMER ORDER BY NAME;
--------------------------------------------------------------------
-- 가격(PRICE)이 10000 미만 -> '싸다'/ 10000-20000 -> '적당하다'/ 20000보다 크면 -> '비싸다'/ 30000보다 크면 -> '너무 비싸다'
SELECT * FROM BOOK;
SELECT B.*, 
