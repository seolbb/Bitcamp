...
*************************************/
-- (관리자계정 - SYSTEM) 유저생성: MDGUEST, 암호: mdguest,
-- 유저생성 + 테이블스페이스 지정
...
-- (MDGUEST2) MDGUEST2 유저가 CUSTOMER 테이블에서 SELECT, UPDATE 가능
----
-- (MADANG) MADANG 유저가 MDGUEST로 부터 권한회수(취소)
REVOKE SELECT, UPDATE ON CUSTOMER FROM MDGUEST; --권한취소
-- MDGUEST 에게 부여된 CUSTOMER 테이블 SELECT, UPDATE 권한이 취소됨
